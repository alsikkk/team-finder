...
> В `docker-compose.yml` по умолчанию используется порт **5433** на хосте (`"5433:5432"`), чтобы не конфликтовать с локальным PostgreSQL на 5432.
> В `.env` должно быть `POSTGRES_PORT=5433`.
...
## Инструкция для ревьюера (вариант 2)

Проект реализован на **Django 5.2** с **PostgreSQL** и шаблонами из папки `templates_var2`.

### Быстрый запуск

1. Скопируйте `.env_example` в `.env`.
2. Запустите базу данных (порт **5433**, чтобы не конфликтовать с локальным PostgreSQL на 5432):
   ```bash
   docker compose up -d
   ```
   В `.env` должно быть `POSTGRES_PORT=5433`.
...
4. Запустите сервер:
   ...
   python manage.py runserver
   ```
5. Откройте [http://127.0.0.1:8000/projects/list/](http://127.0.0.1:8000/projects/list/)

### Тестовые аккаунты

После команды `load_demo_data` доступны пользователи (пароль у всех `demo12345`):

| Email | Роль |
|-------|------|
| nikita.orlov@example.com | Бег, силовые тренировки |
| darya.kozlova@example.com | Плавание, триатлон |
| maksim.belov@example.com | Футбол, баскетбол |

У каждого пользователя есть минимум один проект и набор навыков для проверки фильтрации на `/users/list/`.
